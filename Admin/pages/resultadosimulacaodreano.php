...
<?php
     ...
     $opcao=$_GET['opcao'];
      ...
      $totalcusto_fixo=$totalcusto_fixo+ dre($ano,$i,"CUSTOS FIXOS");

       $resultado_bruto[$i]= dre($ano,$i,"RESULTADO BRUTO");
       

       $despesa_operacional[$i]= dre($ano,$i,"DESPESAS OPERACIONAIS");
        $totaldespesa_operacional=$totaldespesa_operacional+dre($ano,$i,"DESPESAS OPERACIONAIS");
        
       $despesa_administrativa[$i]= dre($ano,$i,"DESPESAS ADMINISTRATIVAS");
        $totaldespesa_administrativa=$totaldespesa_administrativa+ dre($ano,$i,"DESPESAS ADMINISTRATIVAS");
        
       $despesa_comercial[$i]= dre($ano,$i,"DESPESAS COMERCIAL");
        $totaldespesa_comercial=$totaldespesa_comercial+dre($ano,$i,"DESPESAS ADMINISTRATIVAS");
        
         $despesa_comex[$i]= dre($ano,$i,"DESPESAS COMEX");
        $totaldespesa_comex=$totaldespesa_comex+dre($ano,$i,"DESPESAS COMEX");
       ...
       $totaldepreciacao=$totaldepreciacao+dre($ano,$i,"DEPRECIACAO");
       ...
       $resultado_liquido[$i]= dre($ano,$i,"RESULTADO LIQUIDO");
        ...
        if($opcao=="Receita Bruta"){ 
      if ($receita_bruta[$i]>0){
       $porcentagem1=str_replace(",",".", $porcentagem);

     $porcentagem1=$porcentagem1/100;
     $valor=$receita_bruta[$i]+($receita_bruta[$i]*$porcentagem1); 

    

     $devolucao_venda[$i]=($devolucao_venda[$i]*$valor)/$receita_bruta[$i];
    $totaldevolucao_venda= $totaldevolucao_venda+ $devolucao_venda[$i];
    
    $despesa_variavel_venda[$i]=($despesa_variavel_venda[$i]*$valor)/$receita_bruta[$i];
    $totaldespesa_variavel_venda=$totaldespesa_variavel_venda+$despesa_variavel_venda[$i];
    
    $custo_produtos_vendidos[$i]=($custo_produtos_vendidos[$i]*$valor)/$receita_bruta[$i];
    $totalcusto_produtos_vendidos= $totalcusto_produtos_vendidos+ $custo_produtos_vendidos[$i];

     
     
      
         $receita_bruta[$i]=$valor;
   $totalreceita_bruta=$totalreceita_bruta+$receita_bruta[$i];
    
     
     $receita_operacional_bruta[$i]=$receita_bruta[$i]-$devolucao_venda[$i];  
     $totalreceita_operacional_bruta=$totalreceita_operacional_bruta+$receita_operacional_bruta[$i];
      
     
     $receita_operacional_liquida[$i]=$receita_operacional_bruta[$i]-$despesa_variavel_venda[$i];
     ...
     $margem_contribuicao[$i]=$receita_operacional_liquida[$i]-$custo_produtos_vendidos[$i];
     $totalmargem_contribuicao=$totalmargem_contribuicao+$margem_contribuicao[$i];
     
     $resultado_bruto[$i]=$margem_contribuicao[$i]-$custo_fixo[$i];
     $totalresultado_bruto=$totalresultado_bruto+$resultado_bruto[$i];
     $ebita[$i]=$resultado_bruto[$i]-$despesa_operacional[$i];
     $totalebita=$totalebita+$ebita[$i];
     $resultado_antes_juros[$i]=$ebita[$i]-$depreciacao[$i];
     $totalresultado_antes_juros=$totalresultado_antes_juros+$resultado_antes_juros[$i];
     $rair[$i]=$resultado_antes_juros[$i]-(-$resultado_financeiro[$i]);
     $totalrair=$totalrair+$rair[$i];
     $resultado_liquido[$i]=$rair[$i];
        ...
        <div id="page-wrapper">
            ...
            <div class="row">
                    ...
                    <div class="panel panel-primary">
                        ...
                        <center>
                        <h4>
                           Demonstrativo Simulado De Resultados no Ano de <?php echo $mes1?> <?php echo $ano?>
                            </h4>
                            </center>
                        </div>
